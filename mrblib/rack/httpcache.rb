module Rack
  ...
  class HTTPCache
    ...
    # NOTE: temporary exclude 206 Partial Content
    CACHEABLE_STATUSES = [200, 203, 204, 300, 301, 404, 405, 410, 414, 501].map {|s| [s, true] }.to_h

    NOT_MODIFIED_UPDATE_HEADERS = ['Cache-Control', 'Date', 'Expires', 'ETag', 'Last-Modified'].map {|h| h.freeze }

    TRACE_HEADER = 'X-Rack-HTTPCache-Trace'.freeze
    ...
    DEFAULT_OPTIONS = {
      ...
      :default_ttl     => proc {|res|
        if res.last_modified
          ((res.time - res.last_modified) * 0.1).to_i
        else
          3600
        end
      },
    ...
    def call(env)
      req = Request.new(env, {
        :private_headers => @options[:private_headers],
        :cache_key => @options[:cache_key],
      })
      return forward(req).to_a unless req.get? || req.head?
      ...
      begin
        ...
        if cached
          cached = nil unless CACHEABLE_STATUSES.include?(cached.status)
        end
        ...
        message = "cache error: #{e.message}: #{e.backtrace.join(", ")}"
    ...
    def storable?(res, req)
      ...
      return false if res.status == 304
      return false unless CACHEABLE_STATUSES.include?(res.status)
    ...
    def revalidate(cached, req)
      env = req.env.dup
      env[Rack::REQUEST_METHOD] = Rack::GET if req.head?

      env['HTTP_IF_MODIFIED_SINCE'] = cached.last_modified.httpdate if cached.last_modified
